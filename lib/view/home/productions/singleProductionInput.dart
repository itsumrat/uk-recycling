...
class SingleProductionInput extends StatefulWidget {
  ...
  const SingleProductionInput({Key? key, this.grades, this.production, this.existingWeight, this.existingGrade,  this.transactionID, this.existingGradeId}) : super(key: key);
...
class _SingleProductionInputState extends State<SingleProductionInput> {
          ...
          children: [
            ...
            Align(
              ...
              child: InkWell(
                ...
                onTap: ()=>widget.existingWeight != null ? _editProductionWeightGrade() :_addProductionWeightGrade(),
                child: Container(
                  width: 180,
                  height: 70,
                  decoration: BoxDecoration(
  ...
  Future<void> _addProductionWeightGrade() async{
    print("dafds");
    if(_weight.text.isNotEmpty && selectedValue!= null){
      ...
      if(res.statusCode == 200){
        ...
        Get.to(ShowProductionsInputs(transactionID: data["id"].toString(), date: data["created_at"], weight: data["weight"], grade: data["grade"], production: widget.production,));
      }
  ...
  Future<void> _editProductionWeightGrade() async{
    ...
    if(_weight.text.isNotEmpty && selectedValue!= null && widget.production!.id != null){
      ...
      if(res.statusCode == 200){
        ...
        var data = jsonDecode(res.body)["data"];
        Get.to(ShowProductionsInputs(transactionID: widget.transactionID, date: data["created_at"], weight: data["weight"], grade: data["grade"], production: widget.production,));
      }
      setState(() =>isLoading = false);
    }else{
      //show error message
