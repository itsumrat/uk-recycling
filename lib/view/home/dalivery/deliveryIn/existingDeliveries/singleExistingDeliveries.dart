...
class SingleExistingDeliveries extends StatefulWidget {
  ...
  const SingleExistingDeliveries({Key? key, this.existingDeliveryInDatum, this.existingDeliveryId, this.existingDeliveryOutDatum}) : super(key: key);
...
class _SingleExistingDeliveriesState extends State<SingleExistingDeliveries> {
  ...
  Widget build(BuildContext context) {
    return AppWidget(
      appBarOnBack: ()=>Get.to(ExistingDeliveries() ),
      appBarTitle: "${widget.existingDeliveryInDatum!.deliveryInId}/${inputFormat.format(widget.existingDeliveryInDatum!.date!)}/${widget.existingDeliveryInDatum!.supplier!.name}-${widget.existingDeliveryInDatum!.measurement!.name}",
      textSize: 15,
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  //// transactions
                  InkWell(
                    onTap: ()=>Get.to(Transactions(singleDeliveryInModel: snapshot.data!, existingDeliveryInDatum: widget.existingDeliveryInDatum,)),
                    child: Container(
                      width: 200,
                      height: 250,
                  ...
                  SizedBox(height: 30,),
                  InkWell(
                    onTap: ()=>Get.to(CreateNewCageBox(existingDeliveryInDatum: widget.existingDeliveryInDatum, deliveryId: snapshot.data!.data!.delivery!.id.toString(),)),
                    child: Container(
                      width: 200,
                      height: 60,
                      decoration: BoxDecoration(
